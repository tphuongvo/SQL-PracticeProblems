...
SELECT T.yr
    ,T.mth
  ,ROUND(100.0*((T.cnt - ((LAG(T.cnt) OVER(PARTITION BY T.yr ORDER BY T.mth ASC))))/(LAG(T.cnt) OVER(PARTITION BY T.yr ORDER BY T.mth ASC))::numeric),1)  growth_pct
  FROM 
(SELECT 
      EXTRACT(YEAR FROM call_received) yr
    , EXTRACT(MONTH FROM call_received) mth
    , COUNT( policy_holder_id) cnt
  FROM callers
  WHERE call_duration_secs > 300
  GROUP BY EXTRACT(YEAR FROM call_received), EXTRACT(MONTH FROM call_received)
  ORDER BY EXTRACT(YEAR FROM call_received), EXTRACT(MONTH FROM call_received)
  ) T
ORDER BY T.yr, T.mth ASC
;