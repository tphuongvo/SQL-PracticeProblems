/*
UnitedHealth
UnitedHealth Group has a program called Advocate4Me, which allows members to call an advocate and receive support for their health care needs – whether that's behavioural,
...
SELECT 
  COUNT(DISTINCT policy_holder_id) patient_count
FROM
  (
    SELECT policy_holder_id,call_received
    ,EXTRACT(DAY FROM call_received - LAG(call_received) OVER(
                            PARTITION BY(policy_holder_id) ORDER BY call_received)) cnt_day 
    FROM callers
  ) T
WHERE T.cnt_day < 7
;